...
=== `dash-guix` - runs in Dash with Guix environment
...
=== `run-with-term-emu` - runs a command in a termimal emulator

TODO:
* `SHELL` variable and description of this command
* `--detach`

.file::run-with-term-emu
[source,sh]
----
#!/bin/dash

exec term-emu -e "tmux new-session '${SHELL} -c \"$@\"'"
# exec term-emu -e "tmux new-session '${SHELL} -c \"$@\"' \; detach"
...
exec rmacs --new-frame eval "(with-current-buffer (get-buffer-create \"$tempBufferName\")
  ...
  (setq-local local/delete-frame-on-close t)
  (Man-cleanup-manpage)
  ;; (Man-fontify-manpage)
...
Downloads a file, compares checksums, and prompts overwriting when necessary.  This script returns the download destination as the last line upon a successful download with exit code 0.  In case of error or checksums not matching, a non-zero exit code is returned.
...
proc checksumsMatched {path checksumDict} {
	proc checksumMatched {path type checksum} {
		...
		if {$checksum eq ""} {
			...
			switch $type {
				md5 {
					return [expr {$checksum eq [string toupper [::md5::md5 -hex -file $path]]}]
				}
				sha1 {
					return [expr {$checksum eq [string toupper [::sha1::sha1 -hex -file $path]]}]
				}
				sha256 {
					return [expr {$checksum eq [string toupper [::sha256::sha256 -hex -file $path]]}]
				}
				default {
					return 0
				}
...
set checksumDict [dict create md5sum $args(md5sum) sha1sum $args(sha1sum) sha256sum $args(sha256sum)]
...
exec report-missing-executables aria2c Aria2c >@ stdout 2>@ stderr
