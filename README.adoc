...
exec bash --login --noprofile --norc "$@"

----

=== `with-bash-pure` - runs a command under `bash-pure`
...
=== `with-bash-user` - runs a command under `bash-user`
...
=== `with-bash-guix` - runs a command under `bash-guix`
...
=== `run-with-term-emu-detach` - runs and detachs a command in a termimal emulator

.file::run-with-term-emu-detach
[source,sh]
----
#!/usr/bin/env sh
...
=== `run-with-termux` - runs a command with a terminal multiplexer

.file::run-with-termux
[source,sh]
----
#!/usr/bin/env sh
...
Symlink <source> to <destination>.  If <destination> exists, prompt for overwriting.  In case that <source> is a symlink, it is not followed."
	exit 0
fi
...
exec report-missing-executables aria2c Aria2c md5sum Coreutils sha1sum Coreutils sha256sum Coreutils >@ stdout 2>@ stderr
