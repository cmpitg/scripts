...
pidof xkeybinds >/dev/null 2>&1 || exec xbindkeys --nodaemon --poll-rc
...
exec enrich-path wm herbstluft interactive menus - "$@"

----

===== `interactive/perform-action` - performs a high-level WM action

.file::wm/herbstluft/interactive/perform-action
[source,sh]
----
#!/usr/bin/env tclsh
...
set menu [lindex $::argv 0]
set action [exec get-$menu-menu 2> /dev/null | run-context-menu]
...
set ::env(PATH) [stripPrefix [exec enrich-path wm herbstluft interactive menus guix - with-env env | ag {^PATH=}] {^PATH=}]

switch -glob $action {
    "main-menu-call-*" {
        set menuType [stripPrefix $action "main-menu-call-"]
        execl perform-action [list $menuType]
    }
    "wm-*" {
    ...
    "add-tag" {
            ...
            set tagName [string trim [exec yad {--title=Add tag} --entry {--entry-label=Tag name}]]
            if {$tagName ne ""} {
                callHC add $tagName
            }
        }
...
.file::wm/herbstluft/menus/get-app-menu
[source,sh]
----
...
(write
 '(popup-menu (("[_a] Config input" app-config-input)
...
===== `menus/get-client-menu`

.file::wm/herbstluft/menus/get-client-menu
[source,sh]
----
#!/usr/bin/env -S guile -s
# -*- mode: scheme -*-
...
(write
 '(popup-menu (("[_b] Bring here" bring-client-here)
               ("[_j] Jump to" jump-to-client))))

----

===== `menus/get-current-tag-menu`

.file::wm/herbstluft/menus/get-current-tag-menu
[source,sh]
----
#!/usr/bin/env -S guile -s
# -*- mode: scheme -*-
...
(write
 '(popup-menu (("[_r] Rename" rename-current-tag)
               ("[_k] Kill" kill-current-tag))))
      
----

===== `menus/get-focused-client-menu`

.file::wm/herbstluft/menus/get-focused-client-menu
[source,sh]
----
#!/usr/bin/env -S guile -s
# -*- mode: scheme -*-
...
===== `menus/get-frame-menu`

.file::wm/herbstluft/menus/get-frame-menu
[source,sh]
----
#!/usr/bin/env -S guile -s
# -*- mode: scheme -*-
...
(write
 '(popup-menu (("[_u] Split (up)" split-frame-top)
...
.file::wm/herbstluft/menus/get-main-menu
[source,sh]
----
#!/usr/bin/env -S guile -s
# -*- mode: scheme -*-
...
(write
 '(popup-menu (("[_g] Add tag" add-tag)
               ()
               ("[_a] App" main-menu-call-app)
               ("[_f] Frame" main-menu-call-frame)
               ("[_c] Focused client" main-menu-call-focused-client)
               ("[_t] Current tag" main-menu-call-current-tag)
               ()
               ("[_w] WM" main-menu-call-wm))))

----

===== `menus/get-tag-menu`

.file::wm/herbstluft/menus/get-tag-menu
[source,sh]
----
#!/usr/bin/env -S guile -s
# -*- mode: scheme -*-
...
(write
 '(popup-menu (("[_b] Bring here" bring-tag-here)
               ("[_k] Kill" kill-tag))))
...
.file::wm/herbstluft/menus/get-wm-menu
[source,sh]
----
#!/usr/bin/env -S guile -s
# -*- mode: scheme -*-
...
(write
 '(popup-menu (("[_x] Execute client command" wm-exec-client-command)
               ("[_r] Reload" wm-reload)
