...
check-xinput 'PS/2 Synaptics TouchPad' && (
	...
	(
		...
		xinput set-prop 'PS/2 Synaptics TouchPad' 'Evdev Wheel Emulation Axes' 0, 0, 5, 4
	)
)
...
google-chrome --remote-debugging-port=${CHROME_REMOTE_DEBUGGING_PORT:-9222} "$@"
...
run-chrome --app=https://web.whatsapp.com/
...
run-chrome --app=https://calendar.google.com
