...
if {$text ne ""} {
    switch -glob $text {
        ...
        "ssh://*!*" {
            ...
            set cmd [constructEnrichedPathCmd [lindex $parts 1]]
        ...
        "!#*" {
            ...
            execl with-env-user [list env WITHOUT_TERMUX=1 with-term-emu {*}$cmd]
        }
        "!@*" {
            set cmd [constructEnrichedPathCmd [stripPrefix $text {!@}]]
...
=== `gen-random-string` - generates a random alphanumeric string

.file::gen-random-string
[source,sh]
----
#!/usr/bin/env bash
...
=== `gen-random-names` - generates random English names

TODO: docstring

.file::gen-random-names
[source,sh]
----
#!/usr/bin/env bash
...
=== `gen-random-filename` - generates a random filename

TODO: docstring

.file::gen-random-filename
[source,sh]
----
#!/usr/bin/env sh
...
exec normalize-filename $(gen-random-names ${counter_}) | downcase | tr ' ' "${separator_}"
