...
if {$text ne ""} {
    switch -glob $text {
        ...
        "ssh://*!*" {
            set rest [splitString [stripPrefix $text {ssh://}] "!"]
            set hostExpr [string trim [substEnvVars [lindex $rest 0]]]
            set cmd [constructEnrichedPathCmd [lindex $rest 1]]
            execl ssh [list $hostExpr {*}$cmd]
        }
        "ssh://*:*/" {
            set rest [splitString [string trim [stripPrefix $text {ssh://}]] ":"]
            set hostExpr [string trim [substEnvVars [lindex $rest 0]]]
            set path [join [lrange $rest 1 end] ":"]
            execl ssh [list $hostExpr ls -1 --classify --all --group-directories-first $path]
        }
        "ssh://*:*" {
            set rest [splitString [string trim [stripPrefix $text {ssh://}]] ":"]
            set hostExpr [string trim [substEnvVars [lindex $rest 0]]]
