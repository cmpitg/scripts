...
A collection of my scripts.  Many of them are written in
http://doc.cat-v.org/plan_9/4th_edition/papers/rc[Rc shell], for
Bourne-or-Bash-compatible shell suffers from many design flaws, making it
...
and start using it yourself.  All executables are in `bin/`.
...
is set to `/m/9p-fonts/Noto Mono/11a/font`.
...
xinput set-prop 'DELL Laser Mouse' 'libinput Natural Scrolling Enabled' 1
xinput set-prop 'DELL Laser Mouse' 'Evdev Scrolling Distance' -1, -1, 1
xinput set-prop 'DELL Laser Mouse' 'libinput Accel Speed' 0.2

do-notify "Setting natural scrolling for Evoluent Vertical 4"
xinput set-prop 'Kingsis Peripherals Evoluent VerticalMouse 4' 'libinput Natural Scrolling Enabled' 1
xinput set-prop 'Kingsis Peripherals Evoluent VerticalMouse 4' 'Evdev Scrolling Distance' -1, -1, 1
...
xinput set-prop 'MOSART Semi. 2.4G Wireless Mouse' 'Evdev Scrolling Distance' -1, -1, 1
...
scrolling_distance_2_=$(xinput list-props 13 \
	| grep 'Synaptics Scrolling Distance' \
	| gnu-awk '{ print $NF }' \
)
scrolling_distance_1_=$(xinput list-props 13 \
	| grep 'Synaptics Scrolling Distance' \
	| gnu-awk '{ print $(NF - 1) }' \
	| cut -d',' -f1\
)
...
xinput set-prop "${id_}" "Synaptics Scrolling Distance" "-${scrolling_distance_1_}" "-${scrolling_distance_2_}"
...
handling `/open/<file-path>` URI (i.e. command `curl
0:${EMACS_PORT}/open//m/src` opens `/m/src` in Emacs).  Sample setup is as
followed:
...
default, `EMACS_INIT` is set to `"/m/src/emacs-cmpitg/src/init.el"` and
...
report-missing-executables emacs "GNU Emacs" rmacs Rmacs || exit 1
