...
set coreTemps [exec sensors -u coretemp-isa-0000 | ag {temp.*input} | awk {{ printf("%.1f°C, ", $2) }} | sed {s/, $//}]
set coreTempsText "Cores: $coreTemps"

set fanStats [exec sensors -u thinkpad-isa-0000 | ag {fan.*input} | awk {{ printf("%.0f RPMs, ", $2) }} | sed {s/, $//}]
set fanStatsText "Fans: $fanStats"

set batStats [join [exec acpi -b | awk {{ print $4, $3 }} | sed {s/[:,]//g ; s/ Charging/+/g ; s/ Discharging/-/g ; s/ Unknown//g}]]
...
# /tmp/aoeu:10 /hello/
# /tmp/aoeu +10 /hello/
proc tryOpeningFile {editor rest} {
    # Visit a file and return its buffer
    proc visitFile {path} {
        return [exec rmacs --client-opts --alternate-editor=vim --name edit --no-wait visit $path <@ stdin 2>@ stderr]
    }
    
    proc gotoLine {buffer number} {
        return [exec rmacs --client-opts --alternate-editor=vim --name edit --with-buffer $buffer eval "(goto-line $number)" <@ stdin >@ stdout 2>@ stderr]
    }
    
    proc gotoPattern {buffer pattern} {
        return [exec rmacs --client-opts --alternate-editor=vim --name edit --with-buffer $buffer eval "(re-search-forward \"$pattern\")" <@ stdin >@ stdout 2>@ stderr]
    }

    set possiblePath [file normalize [lindex $rest 0]]
    if {[file exists $possiblePath]} {
        set buffer [visitFile $possiblePath]
    } else {
    ...
    foreach arg [lrange $rest 1 end] {
        switch -glob $arg {
            ...
            "/*/" {
                ...
                catch {gotoPattern $buffer $pattern}
            }
            default {
    ...
    exec rmacs --client-opts --alternate-editor=vim --name edit --new-frame open $possiblePath <@ stdin >@ stdout 2>@ stderr
...
cat -s - <(tmux list-sessions -F "#{session_name}" | sed "s/^/tmux :: /g") | exec run-menu-and-dispatch "Switch to"
