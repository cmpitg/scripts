...
=== `9-term` - runs Plan9port terminal emulator within an 9 environment
...
With TODO[@xach's] TODO[Buildapp].  This script takes TODO[ASDF] manifest file
from `manifest` environment variable.  By default, `manifest` is set to `/m/config/common-lisp/sbcl-quicklisp-manifest.txt`.
